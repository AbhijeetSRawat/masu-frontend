...
const AdminPanel = () => {
    ...
    return allItems.map((entry) => {
      if (entry.items) {
        const filtered = entry.items.filter(item =>
          revisedPermissions.includes(item.name) || item.name === "Shift Management"
        );
        return filtered.length ? { subHeading: entry.subHeading, items: filtered } : null;
      }
      return revisedPermissions.includes(entry.name) || entry.name === "Shift Management"
        ? entry
        : null;
    }).filter(Boolean);
  };
  ...
  const renderPermissionCards = () => {
    return Object.entries(structuredPermissions).map(([section, sectionData], i) => {
                ...
                if (entry.items) {
                  ...
                  return (
                    <li key={subKey} className="list-none">
                      <button
                        onClick={() => toggleSubSection(section, entry.subHeading)}
  ...
  return (
    ...
    <div className='w-[100vw] lg:w-[79vw] lg:ml-[20vw]'>
      {/* Header */}
      <div className="w-[100vw] flex justify-center px-6 lg:px-0 py-6 lg:py-0 min-h-[8vh] z-0 top-0 left-9 items-center text-3xl text-white bg-gray-600 font-semibold lg:w-[79vw]">
       <img src={company?.thumbnail} alt="logo" className='h-[6vh] mr-4' /> Admin Panel ({company?.name || 'Company'})
      </div>
      ...
      <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div
          className={`bg-white rounded-lg shadow-md border border-gray-300 p-4 transition-all duration-300 ${
            expandedCards["Department"] ? "shadow-lg scale-[1.02]" : "hover:shadow-lg"
          }`}
        >
          <button
          ...
          >
            ...
            {expandedCards["Department"] ? <MdKeyboardArrowDown /> : <MdKeyboardArrowRight />}
          </button>
        ...
        <div
          className={`bg-white rounded-lg shadow-md border border-gray-300 p-4 transition-all duration-300 ${
            expandedCards["Customize Table"] ? "shadow-lg scale-[1.02]" : "hover:shadow-lg"
          }`}
        >
          <button
          ...
          >
            ...
            {expandedCards["Customize Table"] ? <MdKeyboardArrowDown /> : <MdKeyboardArrowRight />}
          </button>
